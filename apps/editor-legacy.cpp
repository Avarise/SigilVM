...
domain_ui_state_t domain_ui;
domain_backend_t domain_backend;

// Aggregation of main variables
static struct {
    // SigilVM identity descriptors
    sigil::platform::process_descriptor_t proc_info;
    sigil::platform::app_descriptor_t app_info;

    // SigilVM state contexts
    sigil::platform::context_t platform_ctx;
    ...
    bool show_level_editor = false;
    bool show_asset_manager = true;
    ...
    bool show_character_editor = false;
    bool show_initiative_tracker = true;
...
GLFWmonitor* g_last_monitor = nullptr;
...
static std::filesystem::path get_character_dir()
{
    const char* home = getenv("HOME");
    if (!home) return {};

    std::filesystem::path path = home;
    path /= ".local/share/sigilvm/game/characters";
...
static std::string character_to_json(const sigil::game::character_state_t& c)
{
    using namespace sigil::game;

    std::ostringstream out;

    out << "{\n";
    out << "  \"name\": \"" << c.name << "\",\n";
    out << "  \"rest_level\": " << c.rest_level << ",\n";
    out << "  \"attrs\": [\n";

    for (int i = 0; i < (int)attribute_t::count; ++i) {
        out << "    { \"rest\": " << (int)c.attrs[i].rest << " }";
        if (i != (int)attribute_t::count - 1)
            out << ",";
        out << "\n";
    }
...
static sigil::game::character_state_t character_from_json(const std::string& text)
{
    using namespace sigil::game;

    character_state_t c;

    extract_string(text, "\"name\"", c.name);
    extract_int(text, "\"rest_level\"", c.rest_level);

    c.temp_level = c.rest_level;

    size_t pos = text.find("\"attrs\"");
    if (pos != std::string::npos) {
        for (int i = 0; i < (int)attribute_t::count; ++i) {
            pos = text.find("\"rest\"", pos);
            if (pos == std::string::npos) break;

            int value = 0;
            extract_int(text.substr(pos), "\"rest\"", value);

            c.attrs[i].rest = sigil::game::die_clamp(value);
            c.attrs[i].pending_delta = 0;
        }
    }
...
static void draw_initiative_tracker();
static void draw_character_editor();


// Path for .ini file
static std::string get_editor_ini_path();
...
void open_text_editor(const std::filesystem::path& path);
std::vector<file_action_t> resolve_actions(const std::filesystem::path& path);
void dispatch_file_action(file_action_type action, const std::filesystem::path& path);
static int input_text_callback(ImGuiInputTextCallbackData* data);
void read_fd_into_lines(int fd);
...
sigil::game::game_logger_t logger {
    .info = log_info,
    .warn = log_warn,
    .error = log_error
};

sigil::game::initiative_tracker_t tracker {
    .entries = {},
    .current_index = 0,
    .round = 1,
    .logger = &logger,
...
static std::vector<sigil::game::character_state_t> characters;
...
// Rendering Data, to be refactored into SigilVM Render
...
::sigil::yield cmd_help(sigil::platform::cmd_handler_args_t args);

int main(const int argc, const char **argv, const char **envp) {
    // First, creation of app descriptor
    sigil::platform::process_initialize(editor_state.proc_info, argc, argv, envp);
    sigil::platform::app_initialize(editor_state.app_info, editor_state.proc_info);
...
void log_info(const std::string& msg){
    editor_state.output.lines.push_back({ "[info] " + msg, ImVec4(0.7f, 0.7f, 0.7f, 1.0f) });
    editor_state.output.scroll_to_bottom = true;
}

void log_warn(const std::string& msg) {
    editor_state.output.lines.push_back({ "[warn] " + msg, ImVec4(0.9f, 0.7f, 0.2f, 1.0f) });
    editor_state.output.scroll_to_bottom = true;
}

void log_error(const std::string& msg) {
    editor_state.output.lines.push_back({ "[error] " + msg, ImVec4(0.9f, 0.2f, 0.2f, 1.0f) });
    editor_state.output.scroll_to_bottom = true;
}
...
static std::string get_editor_ini_path() {
    ...
    if (!home) {
        // Fallback: no HOME, use local directory deterministically
        return "./editor.ini";
    }

    std::filesystem::path path = std::filesystem::path(home)
        / ".config"
        / "sigilvm";
...
void dispatch_file_action(file_action_type action, const std::filesystem::path& path) {
    switch (action) {
        ...
        {
            editor_state.compat_modal.open = true;
            editor_state.compat_modal.target = path;

            editor_state.compat_modal.prefixes.clear();
            editor_state.compat_modal.tools.clear();
            ...
            for (auto& entry : std::filesystem::directory_iterator(base))
            {
                if (entry.is_directory())
                    editor_state.compat_modal.prefixes.push_back(entry.path());
            }

            // Probe tools (Proton + Wine already handled inside)
            sigil::platform::probe_compat_tools(editor_state.compat_modal.tools);

            editor_state.compat_modal.selected_prefix = -1;
            editor_state.compat_modal.selected_tool = -1;
...
void read_fd_into_lines(int fd) {
    ...
    while ((n = read(fd, buffer, sizeof(buffer))) > 0) {
        for (ssize_t i = 0; i < n; ++i) {
            if (buffer[i] == '\n') {
                editor_state.console.lines.push_back(current);
                current.clear();
            }
            else {
                current += buffer[i];
    ...
    close(fd);
    editor_state.console.scroll_to_bottom = true;
}
...
void execute_command(const std::string& cmd) {
    ...
    if (y.is_failure()) {
        editor_state.console.lines.push_back("[error] execution failed");
        return;
    }
...
void save_text_editor_document(text_editor_document_t& doc) {
...
static ::sigil::yield cmd_gui(const ::sigil::platform::cmd_handler_args_t handler_args) {
    ...
    domain_ui_init(&domain_ui, domain_backend);
    ...
    auto ini_path = ::sigil::platform::get_local_config(editor_state.app_info) / "editor.ini";
    ...
    while (!glfwWindowShouldClose(g_window) && !g_shutdown_requested.load(std::memory_order_acquire)) {
        ...
        wd->ClearValue.color.float32[0] = editor_state.clear_color.x * editor_state.clear_color.w;
        wd->ClearValue.color.float32[1] = editor_state.clear_color.y * editor_state.clear_color.w;
        wd->ClearValue.color.float32[2] = editor_state.clear_color.z * editor_state.clear_color.w;
        wd->ClearValue.color.float32[3] = editor_state.clear_color.w;
...
static void draw_gui() {
    ...
    if (ImGui::BeginMenuBar()) {
        ...
        if (ImGui::BeginMenu("View")) {
            ...
            ImGui::Separator();
            ImGui::MenuItem("Asset Manager", nullptr, &editor_state.show_asset_manager);
            ImGui::MenuItem("File Explorer", nullptr, &editor_state.show_file_explorer);
            ImGui::Separator();
            ImGui::MenuItem("Output", nullptr, &editor_state.show_output);
            ImGui::MenuItem("Preview", nullptr, &editor_state.show_preview);
            ImGui::MenuItem("Terminal", nullptr, &editor_state.show_console);
            ImGui::Separator();
            ImGui::MenuItem("Text Editor", nullptr, &editor_state.show_text_editor);
            ImGui::MenuItem("Level Editor", nullptr, &editor_state.show_level_editor);
            ImGui::MenuItem("Style Editor", nullptr, &editor_state.show_style_editor);
            ImGui::MenuItem("Image Viewer", nullptr, &editor_state.show_image_viewer);
            ImGui::MenuItem("ImGui Demo", nullptr, &editor_state.show_imgui_demo);
            ImGui::Separator();
            ImGui::MenuItem("Initiative Tracker", nullptr, &editor_state.show_initiative_tracker);
            ImGui::MenuItem("Character Editor", nullptr, &editor_state.show_character_editor);
            ImGui::EndMenu();
        }
    ...
    if (editor_state.show_asset_manager)    draw_asset_manager();
    if (editor_state.show_file_explorer)    draw_file_explorer();
    if (editor_state.show_style_editor)     draw_style_editor();
    if (editor_state.show_image_viewer)     draw_image_viewer();
    if (editor_state.show_level_editor)     draw_level_editor();
    if (editor_state.show_text_editor)      draw_text_editor();
    if (editor_state.show_imgui_demo)       draw_imgui_demo();
    if (editor_state.show_console)          draw_console();
    if (editor_state.show_preview)          draw_preview();
    if (editor_state.show_output)           draw_output();
    if (editor_state.compat_modal.open)     draw_compat();
    if (editor_state.show_character_editor) draw_character_editor();
    if (editor_state.show_initiative_tracker) draw_initiative_tracker();



    domain_ui_draw(&domain_ui);
}

static void draw_file_explorer() {
    ImGui::Begin("File Explorer", &editor_state.show_file_explorer);
    ...
    try {
        for (const auto& entry : std::filesystem::directory_iterator(current_dir)) {
            ...
            bool selected =
                editor_state.explorer_selection.valid &&
                editor_state.explorer_selection.path == path;

            if (ImGui::Selectable(label.c_str(), selected)) {
                editor_state.explorer_selection.path = path;
                editor_state.explorer_selection.valid = true;
            }
    ...
    if (editor_state.explorer_selection.valid) {
        auto actions = resolve_actions(editor_state.explorer_selection.path);

        for (const auto& action : actions) {
            if (ImGui::Button(action.label.c_str(), ImVec2(-1, 0))) {
                dispatch_file_action(action.type, editor_state.explorer_selection.path);
            }
        }
    }
    else {
...
static void draw_style_editor() {
    ImGui::Begin("Dear ImGui Style Editor", &editor_state.show_style_editor);
    ImGui::ShowStyleEditor();
    ImGui::End();
}

static void draw_level_editor() {
    ImGui::Begin("Level Editor", &editor_state.show_level_editor);
    ImGui::End();
}
...
//     ImGui::ColorEdit3("clear color", (float*)&editor_state.clear_color); // Edit 3 floats representing a color
...
static void draw_asset_manager() {
    ...
    // --- One-time load ---
    if (!loaded) {
        auto ret = ::sigil::platform::probe_compat_profiles(profiles);
        if (ret.is_failure()) {
    ...
    if (ImGui::BeginListBox("##compat_profiles", ImVec2(-FLT_MIN, 300))) {
        ...
        for (int i = 0; i < (int)profiles.size(); ++i) {
            ...
            if (ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0)) {
                ...
                auto ret = ::sigil::platform::run_compat_profile(profile);
                if (ret.is_failure()) {
...
static void draw_console() {
    ...
    for (const auto& line : editor_state.console.lines) {
        ImGui::TextUnformatted(line.c_str());
    }

    if (editor_state.console.scroll_to_bottom) {
        ImGui::SetScrollHereY(1.0f);
        editor_state.console.scroll_to_bottom = false;
    }

    ImGui::EndChild();

    // Input
    if (editor_state.console.input_buffer.capacity() == 0)
        editor_state.console.input_buffer.reserve(256);

    ImGui::PushItemWidth(-1);

    if (ImGui::InputText(
            "##input",
            editor_state.console.input_buffer.data(),
            editor_state.console.input_buffer.capacity() + 1,
            ImGuiInputTextFlags_EnterReturnsTrue |
            ImGuiInputTextFlags_CallbackResize,
            input_text_callback,
            (void*)&editor_state.console.input_buffer))
    {
        std::string cmd = editor_state.console.input_buffer;

        editor_state.console.lines.push_back("> " + cmd);

        execute_command(cmd);

        editor_state.console.input_buffer.clear();
    }
...
static void draw_imgui_demo() {
    ImGui::ShowDemoWindow(&editor_state.show_imgui_demo);
}

static void draw_text_editor() {
    ImGui::Begin("Text Editor", &editor_state.show_text_editor);
...
static void draw_preview() {
    ImGui::Begin("Preview", &editor_state.show_preview);
    ImGui::End();
}
...
static void draw_output() {
    ...
    {
        editor_state.output.lines.clear();
    }
    ImGui::SameLine();
    ...
    for (const auto& line : editor_state.output.lines)
    {
    ...
    if (editor_state.output.scroll_to_bottom)
    {
        ImGui::SetScrollHereY(1.0f);
        editor_state.output.scroll_to_bottom = false;
    }
...
static void draw_compat() {
    ...
    if (ImGui::BeginPopupModal("Run Executable", &editor_state.compat_modal.open)) {
        ImGui::Text("Target: %s", editor_state.compat_modal.target.string().c_str());
        ImGui::Separator();

        // --- Prefix ---
        ImGui::Text("Select Prefix:");
        for (int i = 0; i < (int)editor_state.compat_modal.prefixes.size(); ++i)
        {
            std::string name = editor_state.compat_modal.prefixes[i].filename().string();

            if (ImGui::Selectable(name.c_str(), editor_state.compat_modal.selected_prefix == i))
                editor_state.compat_modal.selected_prefix = i;
        }
        ...
        for (int i = 0; i < (int)editor_state.compat_modal.tools.size(); ++i)
        {
            auto& tool = editor_state.compat_modal.tools[i];

            if (ImGui::Selectable(tool.name.c_str(), editor_state.compat_modal.selected_tool == i))
                editor_state.compat_modal.selected_tool = i;
        }
        ...
        // --- Run ---
        if (ImGui::Button("Run")) {
            if (editor_state.compat_modal.selected_prefix < 0) {
                log_error("No prefix selected");
            } else if (editor_state.compat_modal.selected_tool < 0) {
                log_error("No runtime selected");
            } else {
                sigil::platform::compat_profile_t profile;

                profile.target = editor_state.compat_modal.target;
                profile.prefix = editor_state.compat_modal.prefixes[editor_state.compat_modal.selected_prefix];
                profile.runner = editor_state.compat_modal.tools[editor_state.compat_modal.selected_tool].path;
                ...
                {
                    auto y = sigil::platform::run_compat_profile(profile);
                    ...
                    editor_state.compat_modal.open = false;
                }
            }
        }
...
static void draw_initiative_tracker()
{
    ...
    ImGui::Begin("Initiative Tracker");

    ImGui::Text("Round: %d", tracker.round);
    ...
    // --- QoL toggles
    static bool show_add_character = false;
    static bool show_add_custom = false;

    ImGui::Checkbox("Add Character", &show_add_character);
    ImGui::SameLine();
    ImGui::Checkbox("Add Entry", &show_add_custom);

    ImGui::Separator();

    // --- Add Character ---
    if (show_add_character) {
        static int selected_character = -1;
        static int new_init_value = 50;

        ImGui::Text("Add Character");

        if (!characters.empty()) {
            if (ImGui::BeginCombo("Character",
                selected_character >= 0 ? characters[selected_character].name.c_str() : "Select")) {

                for (int i = 0; i < (int)characters.size(); ++i) {
                    bool selected = (selected_character == i);
                    if (ImGui::Selectable(characters[i].name.c_str(), selected))
            ...
            ImGui::InputInt("Init##char", &new_init_value);

            if (ImGui::Button("Add##char") && selected_character >= 0) {
                tracker.add_character(&characters[selected_character], new_init_value);
            }
        }
    ...
    if (show_add_custom) {
        static char custom_label[64] = "New Event";
        static int custom_init = 20;

        ImGui::Text("Add Custom Entry");

        ImGui::InputText("Label", custom_label, sizeof(custom_label));
        ImGui::InputInt("Init##custom", &custom_init);

        if (ImGui::Button("Add##custom")) {
            tracker.add_custom(custom_label, custom_init);
        }
    ...
    // --- Entries ---
    for (int i = 0; i < (int)tracker.entries.size(); ++i) {
        auto& e = tracker.entries[i];

        bool is_current = (i == tracker.current_index);

        if (is_current)
            ImGui::Text("> %d - %s", e.initiative, e.display().c_str());
        else
            ImGui::Text("  %d - %s", e.initiative, e.display().c_str());

        if (e.type == entry_type_t::character && e.character) {
            auto& c = *e.character;

            if (ImGui::TreeNode((void*)&c, "%s", c.name.c_str())) {

                for (int a = 0; a < (int)attribute_t::count; ++a) {
                    auto attr = (attribute_t)a;
                    auto& state = c.attrs[a];

                    ImGui::Text("%s: %s (next: %s)",
                        attribute_to_string(attr),
                        die_to_string(state.current_round_value()),
                        die_to_string(state.effective_value())
                    );

                    ImGui::SameLine();

                    std::string plus_id = "+##" + std::to_string(i) + "_" + std::to_string(a);
                    std::string minus_id = "-##" + std::to_string(i) + "_" + std::to_string(a);

                    if (ImGui::Button(plus_id.c_str())) {
                        c.apply_heal(attr, 1);

                        log_info(
                            "Round " + std::to_string(tracker.round) + ": " +
                            c.display_name() + " gained 1 " +
                            attribute_to_string(attr)
                        );
                    }
                    ...
                    if (ImGui::Button(minus_id.c_str())) {
                        c.apply_damage(attr, 1);

                        log_info(
                            "Round " + std::to_string(tracker.round) + ": " +
                            c.display_name() + " took 1 point of " +
                            attribute_to_string(attr) + " damage"
                        );
                    }
                }
...
static void draw_character_editor()
{
    ...
    auto dir = get_character_dir();

    // --- Create
    static char name_buf[64] = "New Character";
    static int level_buf = 0;

    ImGui::Text("Create");
    ImGui::InputText("Name", name_buf, sizeof(name_buf));
    ImGui::InputInt("Level", &level_buf);

    if (ImGui::Button("Create")) {
        character_state_t c;
        c.name = name_buf;
        c.rest_level = level_buf;
        c.temp_level = level_buf;
    ...
    if (ImGui::Button("Import All")) {
        ...
        for (auto& entry : std::filesystem::directory_iterator(dir)) {
            ...
            characters.push_back(character_from_json(buffer.str()));
            imported++;
        }

        log_info("Imported " + std::to_string(imported) + " characters");
    }
    ...
    for (int i = 0; i < (int)characters.size(); ++i) {
        ...
        if (ImGui::TreeNode((void*)&c, "%s", c.display_name().c_str())) {

            ImGui::Text("Rest Level: %d", c.rest_level);
            ImGui::Text("Temp Level: %d", c.temp_level);
            ...
            for (int a = 0; a < (int)attribute_t::count; ++a) {
                ...
                ImGui::Text("%s: %s",
                    attribute_to_string(attr),
                    die_to_string(state.rest)
                );

                ImGui::SameLine();

                std::string plus_id = "+##edit_" + std::to_string(i) + "_" + std::to_string(a);
                std::string minus_id = "-##edit_" + std::to_string(i) + "_" + std::to_string(a);

                if (ImGui::Button(plus_id.c_str()))
                    state.rest = die_clamp((int)state.rest + 1);
            ...
            if (ImGui::Button(("Export##" + std::to_string(i)).c_str())) {
                auto path = dir / (c.name + ".json");

                std::ofstream f(path);
                if (f.is_open()) {
                    f << character_to_json(c);
                    log_info("Exported: " + path.string());
                } else {
                    log_error("Failed to export: " + path.string());
                }
            }
