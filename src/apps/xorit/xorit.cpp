 ...
 * @brief SigilVM Xorit (sigilvm-extra package)
 * @version 1.0
 * @date 2025-12-02
 *
...
int main(const int argc, const char **argv) {
    ...
    ::sigil::fs::file_handler_t key_file(args.keyfile);

    sigil::util::timer_t tm;
    tm.start();

    ::sigil::fs::file_handler_t source(args.infile);
    ...
    tm.start();
    ::sigil::yield st = sigil::utils::xor_encode(source.data, source.file_size, key_file.data, key_file.file_size);
