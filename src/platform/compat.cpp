...
status_t compat_profile_t::validate() {
...
sigil::status_t probe_compat_profiles(std::vector<compat_profile_t> &out) {
    out.clear();
    ...
    for (const auto &e : std::filesystem::directory_iterator(profiles_dir)) {
        ...
        sigil::status_t s = load_compat_profile(e.path(), profile);
...
/*status_t save_profile(const std::filesystem::path &out, compat_profile_t &src) {
