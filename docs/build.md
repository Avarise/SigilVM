...
All intermediate files are generated into `.cache/vm`.  
Final binaries, libraries, and packages are placed into `build/`.

Directory roles:

```
.cache/     → CMake artifacts, temporary state
build/bin/  → compiled executables
build/lib/  → compiled libraries (including libsigilvm.so)
build/pkg/  → generated package contents
```
...
```bash
cmake -B .cache -S . -DCMAKE_BUILD_TYPE=Debug
cmake --build .cache/vm
```
---
...
```bash
cmake -B .cache -S . -DCMAKE_BUILD_TYPE=Release
cmake --build .cache/vm --target all -- -j$(nproc)
```
